# Casos de Uso y Transacciones "Índice Maestro de Pacientes" - Núcleo de Interoperabilidad de Datos (NID) - MINSAL v0.4.9

* [**Table of Contents**](toc.md)
* **Casos de Uso y Transacciones "Índice Maestro de Pacientes"**
...
Diagrama Actores PIXm (Fuente: Perfil IHE)
...
Diagrama Actores PDQm (Fuente: Perfil IHE)
...
Transacción del Caso (Fuente: Perfil IHE)
...
```
GET /Patient/$ihe-pix?sourceIdentifier=urn:oid:2.16.840.1.113883.2.22.0.2.102100.50.10%ACL123456789876
...
Transacciones de 2 Casos de Uso (Fuente: Perfil IHE)
...
```
GET /Patient?_format=json&active=true
...
```
GET /Patient?birthdate=1981-11-10
...
```
GET /Patient?_id=AC1696435881599
...
```
GET /Patient?name=Andres Martinez 
...
```
GET /Patient?telecom=5694332547
...
La respuesta del sistema que suple es un recurso [Bundle](StructureDefinition-BundleSearchMPI.md) de tipo **SearchSet** en donde se indica la cantidad de recursos encontrados con los parámetros coincidentes y cada uno de los éstos incluídos como entrada. 

```
GET /Patient?identifier=15335473-1&birthdate=1981-11-10&name=andres%20martinez
...
La operación se desarrolla basada en un **HTTP POST** utilizando un recurso [Parameters](StructureDefinition-ParametrosEntradaPDQmMpi.md) en el cuerpo, donde uno de sus parámetros es un recurso [Patient](StructureDefinition-MINSALPacienteBusqueda.md), el cuál son los datos que permitirán la búsqueda de los pacientes utilizando la operación [$match](https://build.fhir.org/ig/Minsal-CL/NID/OperationDefinition-MPI.PDQm.match.html).
...
```
 ...
 Ejemplo del uso de la operación **$match** La especificación es como se muestra:
...
El resultado de la operación $match es un recurso [Bundle Searchset](StructureDefinition-BundleSearchMPIMatch.md), el cuál puede entregar uno o más recursos pacientes que correspondan a la busqueda. En el caso de alguna dificultad o problema con la operación se entregará un **Operation Ouctome** dentro de los resultados del Bundle. Ejemplo de un $match exitoso: 
